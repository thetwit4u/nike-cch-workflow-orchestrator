...
export class CchWorkflowOrchestratorStack extends Stack {
    constructor(scope: Construct, id: string, props?: CchWorkflowOrchestratorStackProps) {
        ...
        if (isTestEnv) {
            ...
            // THIS IS THE FIX: Automatically wire the mock queue to the orchestrator
            ...
            commandQueue.grantSendMessages(mockServiceLambda); // Mock service needs to send responses back
            ...
            if (testExecutorArn) {
                ...
                // Allow the principal to tag the session when assuming this role (required for SSO)
        ...
        // Grant permission to the mock queue directly if it exists
        ...
        // Grant orchestrator permission to send to all *other* capability queues
        Object.values(capabilityEnvVars).forEach((queueUrl) => {
            // Skip the mock queue since we handle it with a direct grant above
            if (mockCapabilityQueue && queueUrl === mockCapabilityQueue.queueUrl) {
                return;
            }

            if (queueUrl.includes('sqs')) {
                const queueName = queueUrl.split('/').pop();
                if (queueName) {
                    const queue = sqs.Queue.fromQueueArn(this, `CapabilityQueue-${queueName}`, `arn:aws:sqs:${this.region}:${this.account}:${queueName}`);
                    queue.grantSendMessages(orchestratorLambda);
                }
            }
        });
        ...
        new cdk.CfnOutput(this, 'CheckpointTableName', { value: stateTable.tableName });
