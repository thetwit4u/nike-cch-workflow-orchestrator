...
def test_merged_workflow_happy_path(aws_client, cdk_outputs, stack_name, workflow_verifier, happy_path_s3_uri):
    ...
    # Verify that the merged step outputs are present
    # Should have both enriched delivery set data and filing pack data
    assert "deliverySetImportEnrichedId" in final_context
    assert "deliverySetImportEnrichedURI" in final_context
    assert "deliverySetImportEnrichedStatus" in final_context
    assert "filingPacksStatus" in final_context
    assert "importFilingPacks" in final_context
    
    # Verify the merged step was successful
    assert final_context.get("deliverySetImportEnrichedStatus") == "SUCCESS"
    assert final_context.get("filingPacksStatus") == "SUCCESS"
